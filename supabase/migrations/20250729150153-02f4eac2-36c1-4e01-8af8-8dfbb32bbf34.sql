...
-- 1. Drop the security audit log table and function
...
-- 2. Drop the password validation function
DROP FUNCTION IF EXISTS public.validate_password_security(text);

-- 3. Remove security_level and threat_detected columns from rate_limits table
ALTER TABLE public.rate_limits 
DROP COLUMN IF EXISTS security_level,
DROP COLUMN IF EXISTS threat_detected;

-- 4. Remove security_level and threat_detected columns from enhanced_rate_limits table
ALTER TABLE public.enhanced_rate_limits 
DROP COLUMN IF EXISTS security_level,
DROP COLUMN IF EXISTS threat_detected;

-- 5. Recreate functions without SECURITY DEFINER and search_path restrictions
